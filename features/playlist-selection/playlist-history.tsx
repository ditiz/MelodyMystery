    ...
    <article>
      ...
      <ul className="flex flex-col gap-4">
        {playlists.map((playlistId) => (
          <li key={playlistId} className="flex items-center space-x-3">
            <Link
              href={`/quizz/${playlistId}`}
              className="transition-all font-bold rounded outline outline-offset-2 outline-gray-800 hover:outline-gray-600 px-2 w-80 break-words"
            >
              {playlistId}
            </Link>
          </li>
